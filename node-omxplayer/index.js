...
function buildArgs (source, givenOutput, loop, initialVolume, extraArgs) {
...
function Omx (source, output, loop, initialVolume, extraArgs) {
	...
	function spawnPlayer (src, out, loop, initialVolume, extraArgs) {

		let args = buildArgs(src, out, loop, initialVolume, extraArgs);
	...
	if (source) {
		player = spawnPlayer(source, output, loop, initialVolume, extraArgs);
	}

	// ----- Methods ----- //

	// Restarts omxplayer with a new source.
	omxplayer.newSource = (src, out, loop, initialVolume, extraArgs) => {

		if (open) {

			player.on('close', () => { player = spawnPlayer(src, out, loop, initialVolume, extraArgs); });
			player.removeListener('close', updateStatus);
			writeStdin('q');

		} else {

			player = spawnPlayer(src, out, loop, initialVolume, extraArgs);
